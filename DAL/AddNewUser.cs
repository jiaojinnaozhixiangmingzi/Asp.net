...
namespace DAL
{
    public class AddNewUser : UserRegisterInterface
    {
        ...
        public void ChangePass(string useremail, string newpass)
        {
            ...
            /*md5对密码进行加密*/
            MD5 md5 = new MD5CryptoServiceProvider();
            byte[] data = md5.ComputeHash(System.Text.Encoding.Default.GetBytes(newpass)); ;
            //byte[] md5data = md5.ComputeHash(data);
            md5.Clear();
            ...
            for (int i = 0; i < data.Length - 1; i++)
            {
                UserNewPass += data[i].ToString("x").PadLeft(2, '0');//加密后
            }
            /*md5对密码进行加密*/
            foreach (userinfo t in rusult)
            {

                t.password = UserNewPass;
            }
            ddc.SubmitChanges();
